...
class CpnTemplate:
    ...
    def getCpnSchCfgEventIndex(self):
        """ read file, and get CpnSch config file index. """
        fPath = self.__CpnSchPath + '/' + self.__CpnSchSigsConfigFile
        fReadLines = self.openFileReadLines(fPath)
        fData = ''

        fCpnSchCfgStart = self.getStrLineIndex(self.__CpnSchEvent,fReadLines)
        print('CpnSch Config start index:')
        print(fCpnSchCfgStart)

        fCpnSchCfgEnd = self.getStrBlockEndIndex(self.__CpnSchEvent,fReadLines)
        print('CpnSch Config end index:')
        print(fCpnSchCfgEnd)

        newTrigSig = ('e_%s,' %self.__fileName)
        fCpnSchCfgIndex = self.getStrLineIndex(newTrigSig,fReadLines)
        print('CpnSch Config exist index:')
        print(fCpnSchCfgIndex)

        listIndex = [fCpnSchCfgIndex,fCpnSchCfgStart,fCpnSchCfgEnd]
        return listIndex
    ...
    def addSignalListRegister(self):
        ...
        for i in range(len(fReadLines)):
            ...
            for j in range(len(endIndex)):
                if (0 == existIndex[j]) and (i == endIndex[j]):
                    fData += newLineList[j]
                    continueFlg = 1
                    break
    ...
    def addCpnSchSigsSource(self):
        ...
        newTrigSig = ('sig_trig_%s_init' %self.__fileName)
        newLine = ('    SYN_TRIG('+newTrigSig+', OOPC_NULL);\n')
    ...
    def addCpnSchCfg(self):
        """ read and modify files, new added initial signal register in CpnSch to be initialed. """
        fPath = self.__CpnSchPath + '/' + self.__CpnSchSigsConfigFile
        fReadLines = self.openFileReadLines(fPath)
        fData = ''

        indexList = self.getCpnSchCfgEventIndex()
        existIndex = indexList[0]
        startIndex = indexList[1]
        endIndex = indexList[2]
        print(indexList)
