...
void serv_CpnMem_memMalloc(void *buff){
    ...
    if(memPointer->indication){
        memPointer->pointer = clBlockM.malloc(clBlockM.self, (uint16)memPointer->indication);
    }
}
/* memFree Service.
 **********************************************/
...
#endif
    uint32 i = 0U, j = 0U, ulMaxIndex = 0U;
    static uint32 uwLastCounter = 0u;
...
//#define POINTER_ALIGN  -8lX
    ...
    CPNMEMCFG_MEM_DBG_INFO("blockMMaskMaskMaskMax:%lu  blockMMaskMaskMax:%lu  blockMMaskMax:%lu  blockMMax:%lu  blockMCounterDiff:%u\n",
            pclBlockM->blockMMaskMaskMaskMax,pclBlockM->blockMMaskMaskMax,pclBlockM->blockMMaskMax,pclBlockM->blockMMax,(uint16)i);

    /* every mask details */
    CPNMEMCFG_MEM_DBG_INFO("\nEvery blockMMaskMaskMask detail as follow:\n");
    for(i = 0U; i < pclBlockM->blockMMaskMaskMaskMax; i++){
        CPNMEMCFG_MEM_DBG_INFO("[%-3u]:0x%-8lX ", (uint16)i, pclBlockM->blockMMaskMaskMask[i]);
        if((i&0x7u)==0x7u){
            CPNMEMCFG_MEM_DBG_INFO("\n");
        }
    }
    ...
    for(i = 0U; i < pclBlockM->blockMMaskMaskMax; i++){
        CPNMEMCFG_MEM_DBG_INFO("[%-3u]:0x%-8lX ", (uint16)i, pclBlockM->blockMMaskMask[i]);
        if((i&0x7u)==0x7u){
            CPNMEMCFG_MEM_DBG_INFO("\n");
        }
    }
    ...
    while(i<pclBlockM->blockMMaskMax){
        ...
        for(j = i; j < ulMaxIndex; j++){
            CPNMEMCFG_MEM_DBG_INFO("[%-3u]:0x%-8lX ", (uint16)j, pclBlockM->blockMMask[j]);
        }
        CPNMEMCFG_MEM_DBG_INFO("\n");
        for(j = i; j < ulMaxIndex; j++){
            CPNMEMCFG_MEM_DBG_INFO("[%-3u]:0x%-8lX ", (uint16)j, pclBlockM->blockMMaskContinue[j]);
        }
        CPNMEMCFG_MEM_DBG_INFO("\n");
        i = ulMaxIndex;
    }
