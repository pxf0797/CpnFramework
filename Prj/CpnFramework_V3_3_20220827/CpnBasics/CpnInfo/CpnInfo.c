...
void CpnInfo_run(hCpnInfo cthis){
    ...
    case INFO_SENDING:
        if(tsInfo.uwUsedCnt > CPNINFOCFG_OUTPUT_FRAME_SIZE){
            cthis->infoout.infoout((uint8*)tsInfo.pBuff, CPNINFOCFG_OUTPUT_FRAME_SIZE);
            tsInfo.uwUsedCnt -= CPNINFOCFG_OUTPUT_FRAME_SIZE;
            tsInfo.pBuff += CPNINFOCFG_OUTPUT_FRAME_SIZE;
            teStep = INFO_SENDING;
        }else{
            cthis->infoout.infoout((uint8*)tsInfo.pBuff, tsInfo.uwUsedCnt);
            tsInfo.uwUsedCnt = 0;
            tsInfo.pBuff = NULL;
