...
void CpnSourceTest_testSource(hCpnSourceTest cthis){
    ...
    ASYN_SET(sig_set_CpnSourceTest_wOutput, &wSine2Output);

    /* wait delay */
    SYN_TRIG(sig_trig_CpnSourceTest_delay, &waitDelay);
    if(IS_TIMEOUT(waitDelay)){
        ...
        //wSine2.duty = 800;
        wSine2.phase = 1800;
        SYN_TRIG(sig_trig_CpnSourceTest_changeWaveSig, &wSine2);
    }
}
