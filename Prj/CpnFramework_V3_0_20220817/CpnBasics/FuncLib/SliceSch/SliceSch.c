...
void SliceSch_run(hSliceSch cthis){
    ...
    */
    ...
    for(i = 0u; i < cthis->slicePrd; i++){
        ...
        while(actGroupIndex < cthis->sliceNums){
            cthis->sliceFunc(actGroupIndex);
            actGroupIndex += cthis->schPrd;
        }
    }
 ...
 **********************************************/
hSliceSch SliceSch_init(hSliceSch cthis, uint16 sliceNums, uint16 slicePrd, uint16 schPrd,
    ...
    do{
        ...
        */
        if(!(sliceFunc && sliceNums && slicePrd && schPrd && sliceTable)){
            retRes = OOPC_NULL;
            break;
        }
        /* Configure functions. */
