...
#define appMemCopy memcpy
#else
#define appMemCopy(destination, source, len) do{                 \
...
#else
#define appMemSet(destination, data, len) do{                    \
    uint8 *pucDest = (uint8 *)(destination);                  \
    uint16 i = 0U;                                            \
    while(i < len){                                           \
        pucDest[i] = data;                                    \
 ...
 * Input    :
 * Output   :
 * Others   : convert macro and data to string.
 **********************************************/
#define TOSTR(s)                               #s
#define DATATOSTR(s)                           TOSTR(s)
