...
/* Data definition.
 **********************************************/
//static uint8 memSpace[BLOCKMCFG_CAL_MANAGE_SIZE(uint32,CPNSCH_MEM_SPACE_SIZE)];
...
void CpnSch_err(hCpnSch cthis, uint16 code){
    uint32 uwErrCode = (((uint32)CPNSCHCFG_ERR_ID<<16u) + code);
    clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnSch_setErr, (uint8*)&uwErrCode);
}
