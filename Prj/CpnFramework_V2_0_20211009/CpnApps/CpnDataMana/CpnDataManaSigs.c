...
void serv_CpnDataMana_getDataLen(void *buff){
    ...
    if(len){
        memCopy(&dmDataSetsSpace[dmDataSetsStartAddr[dataLen->pduId]+dataLen->index],dataLen->buff,len);
    }
}
/* setDataSet Service.
 **********************************************/
void serv_CpnDataMana_setDataSet(void *buff){
    htsDataSet dataSet = (htsDataSet)buff;
    memCopy(&dmDataSetsSpace[dmDataSetsStartAddr[dataSet->pduId]],dataSet->dataSet,dmDataSets[dataSet->pduId].dataSetLen);
...
void serv_CpnDataMana_setDataLen(void *buff){
    ...
    /* if length is abnormal stop copy datas. */
    if(len){
        memCopy(dataLen->buff,&dmDataSetsSpace[dmDataSetsStartAddr[dataLen->pduId]+dataLen->index],len);
    }
}
