...
void CpnSource_run(hCpnSource cthis){
    ...
    for(i = 0u; i < CPNSOURCECFG_WAVE_TYPE_GROUPS; i++){
        ...
        while(groupMask){
            ...
            if((cthis->waveGroups[i].sourceParams->step)&&(cthis->waveGroups[i].sourceCounter[curIndex])){
                if((cthis->waveGroups[i].sourceCounter[curIndex]%cthis->waveGroups[i].sourceParams->step)){
                    continue;
                }
            }

            /* output wave. */
            switch (cthis->waveGroups[i].sourceParams->type)
            {
            case w_sine:
...
void CpnSource_err(hCpnSource cthis, uint16 code){
    uint32 uwErrCode = (((uint32)CPNSOURCECFG_ERR_ID<<16u)+code);
