    ...
    for(i = 0u; i < CPNSINKSCFG_CACHE_TYPE_GROUPS; i++){
        ...
        while(groupMask){
            ...
            && (++cthis->cacheGroups[i].cacheCounter[curIndex] >= cthis->cacheGroups[i].sinksParams[curIndex].period)){ /* Increase counter. */
                ...
                /* Fetch datas. */
                ASYN_ADDR(cthis->cacheGroups[i].sinksParams->get_getSig, &addr); /* Get data */
                cthis->cacheGroups[i].cacheFifo[curIndex].push(cthis->cacheGroups[i].cacheFifo[curIndex].self, addr);
...
void CpnSinks_cacheEnd(hCpnSinks cthis, htSinksParams sinks){
    SYN_TRIG(sinks->trig_endSig, OOPC_NULL);
}
...
void CpnSinks_registerCache(hCpnSinks cthis, htSinksParams sinks){
    ...
    for(i = 0u; i < CPNSINKSCFG_CACHE_TYPE_GROUPS; i++){
        ...
        if((curMask+1u)!=0u){
            ...
            cthis->cacheGroups[i].startMask &= (~curMask);
            curIndex = log_2n((uint32)curMask);
            sinks->type_index = (16u*i+curIndex);
            ...
            /* Initial Fifo. */
            CN(Fifo,&cthis->cacheGroups[i].cacheFifo[curIndex],sinks->buff,sinks->buff_size,sinks->data_size);
...
void CpnSinks_err(hCpnSinks cthis, uint16 code){
    tsErrCode errCode;
    errCode.code = code;
    errCode.id = CPNSINKSCFG_ERR_ID;
    SYN_TRIG(sig_trig_CpnSinks_setErr, &errCode);
}
