...
//        appMemCopy(tsFrame.frame->byte, buff, CPNINFOCFG_OUTPUT_FRAME_SIZE);
//    }else{
//        appMemCopy(tsFrame.frame->byte, buff, len);
//        memSet(&tsFrame.frame->byte[len], 0, (CPNINFOCFG_OUTPUT_FRAME_SIZE-len));
//    }
//
    // SYN_TRIG(sig_trig_CpnInfo_updateFrame, &tsFrame);
    // SYN_TRIG(sig_trig_CpnInfo_sendFrame, &tsFrame.id);
}
...
void CpnInfo_setErr(hCpnInfo cthis, uint16 code){
    tsErrCode errCode;
    errCode.code = code;
    errCode.id = CPNINFOCFG_ERR_ID;
    SYN_TRIG(sig_trig_CpnInfo_setErr, &errCode);
}
...
void CpnInfo_info(hCpnInfo cthis, const char *file, const char *func,
    ...
    SYN_TRIG(sig_trig_CpnInfo_now, &uwNow);
