...
typedef struct {
    uint16 active      : 1u;
    uint16 lowestGroup : 3u;
    uint16 rsvd        : 12u;
    uint16 groupIndex;      /* in the schedule group index. or lowest group index. */
...
#define IS_TIMEOUT(tick)          ((delayTick)tick).bit.expired
/* define slice macros */
...
#define SLICE_TASK_DF(basic,period,num)           void task_##period##ms_basic_slice(uint16 index){ \
    uint16 groupIndex = (index>>GROUP_TYPE_BITS);                                                   \
    taskGroupType groupMask = (1ul<<(index&GROUP_TYPE_MASK));                                       \
...
#define BASIC_TASK_DF(basic,period,num)           void task_##period##ms_basic(void){ \
