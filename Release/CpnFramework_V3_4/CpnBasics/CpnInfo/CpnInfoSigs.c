...
void serv_CpnInfo_init(void){
#if (CPNINFOCFG_REPORT_ENABLE == TRUE)
    CN(CpnInfo,&clCpnInfo,&clBlockM,ucInfoSpace,sizeof(tsManaType),
    CPNINFOCFG_MEM_ALIGN_SIZE/sizeof(tsManaType),trig_CpnInfo_inf_infoout);
    if(OPRS(clCpnInfo) != NULL){
        ...
        SYN_TRIG(sig_trig_CpnInfo_initTask, &taskParam);
    }
#endif
}
