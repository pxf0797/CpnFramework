...
#define LEVEL_LIST(_)                                \
    ...
    _(1,   1) /* Configure level1 task groups. */    \
...
/* configure lowest group task counters. */
/* basic slice period MS_T, task period MS_T, predefine task nums */
#define SLICE_LIST(_)              \
    _( 3,     5,    16)            \
    _( 3,    10,    16)            \
    _( 3,    20,     8)            \
...
/* configure red black tree max counter to store the task initial parameters. */
#define CPNSCH_INIT_TASK_MAX_NUM                     32u                                     /* Initial tasks max counter. if it is not big enough, you can expansion. */
