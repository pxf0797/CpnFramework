...
CL(Fifo){
    ...
    uint16 currCnt;                                            /* Cache queue cached counters. */
    uint16 top;                                                /* Cache queue new cache position. */
    uint16 bottom;                                             /* Cache queue first cache position. */

    /* Fifo class functions. */
    uint8 (*push)(hFifo cthis, void *data);                       /* Object cache in cache queue. */
    uint8 (*pop)(hFifo cthis, void *data);                        /* Last object out cache queue. */
    uint8 (*fout)(hFifo cthis, void *data);                       /* First object out cache queue. */
    uint8 (*del)(hFifo cthis, void *data, void *fifoObjTempBuff); /* Delete a certain data in the queue; in order to delete the data,
                                                                     an object temporary buffer area FifoObjTempBuff needs to be provided. */
    uint8 (*isFull)(hFifo cthis);                                 /* Cache queue is full. TRUE/FALSE */
    uint8 (*isEmpty)(hFifo cthis);                                /* Cache queue is empty. TRUE/FALSE */
    uint8 (*clearAll)(hFifo cthis);                               /* Clear all cache queue objects. TRUE/FALSE */
};
