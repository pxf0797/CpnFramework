...
* Description  : CpnMemTest类各功能函数定义源文件
* Others       :
...
#define MEM_TEST_BYTE_SIZE     8U
static TS_MEM_POINTER memPointer[MEM_TEST_BUFF_MAX];

#define TO_STR(s) #s
#define TO_FILE_STR(s)  "D:\\CpnMemTest\\"TO_STR(s)"_"__DATE__".txt" //"D:\\CpnMemTest\\"TO_STR(s)"_"__DATE__"_"__TIME__".txt" //"D:\\CpnMemTest\\CpnMemTest.txt"//
typedef enum{
    /* big end */
...
#define INCREASE_DELAYS() do{\
        clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_delay, (uint8*)&ulWaitTime); \
...
//            clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_memStatus, (uint8*)fp); \
//            te_test_op = testing; \
...
#define OpenAndCloseFile(test_case) do{ \
    if(te_test_op == open_file){ \
            ...
            uwDelayS = 0u; \
            ulWaitTime = 1000*MS_T; \
            memset(memPointer,0,sizeof(TS_MEM_POINTER)*MEM_TEST_BUFF_MAX);\
        }else if(te_test_op == close_file){ \
...
//#endif
    ...
    for(i = 0u; i < MEM_TEST_BUFF_MAX; i++){
        ...
        clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_isPointerSet, (uint8*)(&dataTempBuf));
        CPNMEMCFG_MEM_DBG_INFO("[%u]:0x%-8lX  isSet:%u  ", i, dataTempBuf.pointer, dataTempBuf.indication);
        clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_getIndex, (uint8*)(&dataTempBuf));
        CPNMEMCFG_MEM_DBG_INFO("index:%u  ", dataTempBuf.indication);
        clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_getSize, (uint8*)(&dataTempBuf));
        CPNMEMCFG_MEM_DBG_INFO("size:%u  ", dataTempBuf.indication);
        if((i&0x3u)==0x3u){
...
* CpnMemTest类定义
***********************************************************/
/*名称 : CpnMemTest_run(hCpnMemTest cthis)
* 输入 : cthis - CpnMemTest类指针
* 输出 : 无
* 其他 : CpnMemTest类功能函数
***********************************************/
        ...
        if(te_test_op == testing){
            if(te_testing == test_up){
                if(index < MEM_TEST_BUFF_MAX){
                    ...
                    clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_memMalloc, (uint8*)&memPointer[index]);
                    clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_memStatus, (uint8*)fp);
                    PointerInfo();
                    index++;
//                    if(index == 20){
...
//                    }
                ...
                if(index < MEM_TEST_BUFF_MAX){
                    clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_memFree, (uint8*)(memPointer[index].pointer));
                    clRteSynSigs.trig(clRteSynSigs.self, sig_trig_CpnMemTest_memStatus, (uint8*)fp);
                    PointerInfo();
                    index++;
                }else{
...
/*名称 : hCpnMemTest CpnMemTest_init(hCpnMemTest cthis)
* 输入 : cthis - CpnMemTest类指针
* 输出 : hCpnMemTest - cthis/OOPC_NULL
* 其他 : CpnMemTest类初始化函数
***********************************************/
...
/*名称 : hCpnMemTest CpnMemTest_ctor(hCpnMemTest cthis)
* 输入 : cthis - CpnMemTest类指针
* 输出 : hCpnMemTest - cthis/OOPC_NULL
* 其他 : CpnMemTest类构造函数
***********************************************/
...
/*名称 : hCpnMemTest CpnMemTest_dtor(hCpnMemTest cthis)
* 输入 : cthis - CpnMemTest类指针
