...
void * BlockM_malloc(hBlockM cthis, uint16 size){
    ...
    /* get pointer */
    if(uwIndex<cthis->blockMMax){
        pointer = cthis->getAddr(cthis,uwIndex);
    }
    return pointer;
}
...
uint16 BlockM_free(hBlockM cthis, void *pointer){
    ...
    if(uwIndex<cthis->blockMMax){
        if(cthis->isIndexSet(cthis,uwIndex)){
            if(cthis->isIndexContinueSet(cthis,uwIndex)){
...
uint16 BlockM_findMinIndex(hBlockM cthis){
    ...
    for(i = 0; i < cthis->blockMMaskMaskMax; i++){
        ...
        if(uwMaskIndex>=cthis->blockMMax){ /* Overflow return */
            break;
        }
...
static inline void BlockM_maskContinue(hBlockM cthis,blockMType *ulBlockMask,
    uint16 *uwIndex, uint16 *uwCountResidue, uint16 count, uint16 uwGroupIndex){
    uint16 uwMaskIndex = 0U;
    ...
    if(*uwCountResidue == count){
        *uwIndex = (uwMaskIndex+(uwGroupIndex*(sizeof(blockMType)<<3u)));
        if(((*uwIndex)+count)>cthis->blockMMax){
    ...
    if(((sizeof(blockMType)<<3u)-uwMaskIndex) >= *uwCountResidue){
        ...
        *uwCountResidue = (*uwCountResidue-((sizeof(blockMType)<<3u)-uwMaskIndex)); /* find residue blank */
    }
 ...
 * Others   : BlockM class function.
 **********************************************/
uint16 BlockM_isIndexContinueSet(hBlockM cthis, uint16 index){
    uint16 uwRet = FALSE;
    uint16 uwIndex = 0u;
