...
static inline void BlockM_maskContinue(hBlockM cthis,blockMType *ulBlockMask,
    ...
    if(((sizeof(blockMType)<<3u) - ulMaskIndex) >= (*uwCountResidue)){
        uwShift = (((sizeof(blockMType)<<3u) - ulMaskIndex) - (*uwCountResidue));
        ulContinueMask = ((ulContinueMask << uwShift) >> uwShift);
        *uwCountResidue = 0u;
    }else{
        /* find residue blank */
        *uwCountResidue = (*uwCountResidue-((sizeof(blockMType)<<3u)-ulMaskIndex));
    }
        ...
        case GET_SIZE_BEGIN:
            do{
                ...
                if(ulBlockMask){
                    ...
                    if(ulMask){
                        ulMask = MASK_FIRST_BIT(ulMask);
                        uwSize = (log_2n(ulMask)-ulIndex);
                    }else{
                        uwSize = ((sizeof(blockMType)<<3u)-ulIndex);
                    }
                    teGetStep = GET_SIZE_END;
                    break;
                }
