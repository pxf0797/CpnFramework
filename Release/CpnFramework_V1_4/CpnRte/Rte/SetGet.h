...
/* Signals definition.
 **********************************************/
#define SIGNAL_SET_DEFINE(signal_set,len)                       signal_set,                                             /* Sent signal name definition. */
#define SIGNAL_GET_DEFINE(signal_get,signal_set)                signal_get,                                             /* Receive signal name definition. */
#define SIGNAL_SET_LEN_DEFINE(signal_set,len)                   signal_set##Len = len,                                  /* Sent signal length definition. */
#define SIGNAL_GET_INDEX_MAP_DEFINE(signal_get,signal_set)      signal_get##Map = signal_set,                           /* Receive signal maps. */
#define SIGNAL_SET_LEN_ADD_DEFINE(signal_set,len)               +signal_set##Len                                        /* Sent signal length superposition. */
#define SIGNAL_SET_REGISTER(name,signal_set,len)                if(signal_set){name##SigSetIndex[signal_set] = (signal_set##Len+name##SigSetIndex[signal_set-1]);}\
                                                                else{name##SigSetIndex[signal_set] = signal_set##Len;}  /* Sent signal register in array. */
...
/* Sent Signals length definition.
 **********************************************/
